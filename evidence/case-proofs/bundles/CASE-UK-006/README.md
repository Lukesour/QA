...
## Owner Notes
- 背景：商科应届，文书初稿偏流水账；经历多但主线弱、匹配度不清。
- 动作：改为“定位-证据-落地”三段式，逐段补量化事实；上传结构图与段落修改示例。
- 结果：文书自评结构分从 `__OWNER_FILL_分__` 提升到 `__OWNER_FILL_分__`；项目匹配关键词覆盖从 `__OWNER_FILL_%__` 提升到 `__OWNER_FILL_%__`。
